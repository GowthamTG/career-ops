...
// (plugins.local/webintel/test/*.suite.mjs, named .suite so the layout check
// does not expect a runner for them) from test-all.mjs, which only
...
test('webintel plugin suite', { skip: !existsSync(DIR) && 'plugins.local/webintel not installed' }, () => {
  const files = readdirSync(DIR).filter((f) => f.endsWith('.suite.mjs')).map((f) => path.join(DIR, f));
