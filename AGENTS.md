...
| `basic-validate-pipeline.mjs` | Zero-LLM PASS/FAIL gate for `data/pipeline.md` (title/archetype relevance, location, years-of-experience, comp floor) — the deterministic replacement for full `oferta` evaluation per the house rule in `modes/_custom.md`; annotates rows with `\| gate: PASS/FAIL — {reason}`, additive and idempotent like `rank-pipeline.mjs`. Never submits or drafts an application itself |
| `rank-pipeline.mjs` | Opt-in LLM relevance re-ranker for `data/pipeline.md` — annotates `\| rank: {score}/5 — {reason}`, never filters or deletes a row; bounded cost via `--limit` |
